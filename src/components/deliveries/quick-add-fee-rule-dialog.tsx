...
interface QuickAddFeeRuleDialogProps {
  open: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export const QuickAddFeeRuleDialog = ({ open, onClose, onSuccess }: QuickAddFeeRuleDialogProps) => {
  const { toast } = useToast();
  const { createRule } = useDeliveryFeeRules('');

  const [formData, setFormData] = useState({
    name: '',
    description: '',
    feeType: 'fixed' as 'fixed' | 'by_zone' | 'by_distance' | 'combination',
    fixedFeeAmount: 0,
    distanceBaseFee: 0,
    ...
    applyToProviderType: 'all' as 'all' | 'internal' | 'external',
    isPeakHours: false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Regla creada',
        description: `La regla ${formData.name} se creó exitosamente`
      });
      onClose();
      onSuccess?.();
      setFormData({
      ...
      toast({
        variant: 'destructive',
        title: 'Error',
        description: error instanceof Error ? error.message : 'No se pudo crear la regla de tarifa'
      });
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Agregar Nueva Regla de Tarifa</DialogTitle>
        </DialogHeader>
              ...
              <>
                ...
                <div className="col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, peakHoursMultiplier: parseFloat(e.target.value) || 1.5 })}
                  />
                </div>
              </>
            )}

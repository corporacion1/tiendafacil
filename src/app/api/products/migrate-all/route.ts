...
/**
 * POST - Migrar TODOS los productos de una tienda a múltiples imágenes
 */
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🎉 [API] Migración masiva completada: ${migratedCount}/${productsToMigrate.length} productos migrados`);
    
    return NextResponse.json({
      success: true,
      message: `Migración masiva completada: ${migratedCount}/${productsToMigrate.length} productos migrados`,
      migratedCount,
      totalFound: productsToMigrate.length,
...
/**
 * GET - Verificar cuántos productos necesitan migración
 */
export async function GET(request: NextRequest) {
  try {
    ...
    return handleDatabaseError(error, 'GET migrate all products status');
  }
}